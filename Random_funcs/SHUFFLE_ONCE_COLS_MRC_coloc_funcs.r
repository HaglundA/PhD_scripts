...
MRC_prep_coloc=function(Step1=TRUE,
 ...
 "\n filter_pseudobulk_thresh=",filter_pseudobulk_thresh))
  ...
  geno_loc<-read.table("snp_chromlocations.csv")
  ...
  geno_mat<-geno_mat[colnames(exp_list[[1]])]
  exp_list[[1]]<-exp_list[[1]][sample(ncol(exp_list[[1]]))]

  message("shuffling colname once ")

  exp_list<-lapply(exp_list,function(x){
    return(x[colnames(exp_list[[1]])])
  })
